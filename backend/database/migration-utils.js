...
async function migrateTableData() {
    ...
    try {
        ...
        console.log('Applications migration error (might not exist):', error.message);
    }
}
...
export async function finalizeTableMigration() {
    ...
    try {
        ...
        await executeQuery('ALTER TABLE jobs ADD CONSTRAINT fk_jobs_posted_by FOREIGN KEY (posted_by) REFERENCES users(id) ON DELETE CASCADE');
        await executeQuery('ALTER TABLE applications ADD CONSTRAINT fk_applications_job_seeker FOREIGN KEY (job_seeker_user_id) REFERENCES users(id) ON DELETE CASCADE');
        await executeQuery('ALTER TABLE applications ADD CONSTRAINT fk_applications_employer FOREIGN KEY (employer_user_id) REFERENCES users(id) ON DELETE CASCADE');
        await executeQuery('ALTER TABLE applications ADD CONSTRAINT fk_applications_job FOREIGN KEY (job_id) REFERENCES jobs(id) ON DELETE CASCADE');
        ...
        await executeQuery('CREATE INDEX IF NOT EXISTS idx_jobs_posted_by ON jobs(posted_by)');
        await executeQuery('CREATE INDEX IF NOT EXISTS idx_jobs_job_niche ON jobs(job_niche)');
