import { executeQuery, executeTransaction } from '../database/pg-raw-connection.js';
...
export const ApplicationModel = {
    ...
    async create(applicationData) {
        const {
            jobSeekerUserId, jobSeekerName, jobSeekerEmail, jobSeekerPhone,
            jobSeekerAddress, resumePublicId, resumeUrl, coverLetter,
            employerUserId, jobId, jobTitle
        } = applicationData;
        ...
        const values = [
            jobSeekerUserId, jobSeekerName, jobSeekerEmail, jobSeekerPhone,
            jobSeekerAddress, resumePublicId || null, resumeUrl || null, coverLetter,
            'Job Seeker', employerUserId, 'Employer', jobId, jobTitle
        ];
    ...
    async findByIdWithDetails(id) {
        ...
        const result = await executeQuery(query, [id]);
        return result.rows[0] ? this.formatApplicationWithDetails(result.rows[0]) : null;
    },
    ...
    validateApplicationData(applicationData) {
        ...
        if (!applicationData.jobSeekerName || applicationData.jobSeekerName.trim().length === 0) {
            errors.push('Job seeker name is required');
        }

        if (!applicationData.jobSeekerEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(applicationData.jobSeekerEmail)) {
        ...
        if (!applicationData.jobSeekerPhone) {
            ...
            if (phoneNumber === null) {
                errors.push('Please provide a valid job seeker phone number (10-15 digits)');
            }
        }

        if (!applicationData.jobSeekerAddress || applicationData.jobSeekerAddress.trim().length === 0) {
        ...
        if (!applicationData.jobTitle || applicationData.jobTitle.trim().length === 0) {
